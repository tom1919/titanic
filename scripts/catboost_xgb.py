...
print(time.time() - start) #  Run time: 29.436 seconds
...
print(end - start) # Run time: 1.112 seconds

#%%
# Make predictions on the test / submission set

pred = xgb_mod.fit(X,Y).predict(test2.drop(['Survived'], axis = 1))

pd.DataFrame({'PassengerID' : test.PassengerId, 
              'Survived' : pred.astype(int)}).to_csv(
'C:/Users/tommy/Google Drive/titanic/data/xgb.csv', index = False)

pred2 = cat_mod.fit(X,Y).predict(test2.drop(['Survived'], axis = 1))

pd.DataFrame({'PassengerID' : test.PassengerId, 
              'Survived' : pred2.astype(int)}).to_csv(
'C:/Users/tommy/Google Drive/titanic/data/cb3.csv', index = False)
